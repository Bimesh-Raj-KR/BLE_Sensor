...
public:
    SystemSetting(SysConfig &config, QWidget *parent = 0, Qt::WindowFlags flags = Qt::Widget);
    ~SystemSetting();

    SysConfig getSysConfig(){return m_syscfg;}

signals:
    void saveLayout(QString);
...
private slots:
    ...
    void slotSaveLayout();
    void slotLoadLayout();
    void slotSaveLayoutResult(int ret, QString err);
    void slotLoadLayoutResult(int ret, QString err);
