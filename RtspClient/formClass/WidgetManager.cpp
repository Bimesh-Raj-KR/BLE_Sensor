...
void WidgetManager::getLayout(int & rows, int & cols, LayoutZoneList & zones)
{
    ...
    for (int i = 0; i < m_WidgetLayout.zones.size(); i++)
    {
        if (m_WidgetLayout.zones[i].widget)
        {
            m_WidgetLayout.zones[i].zone.url = m_WidgetLayout.zones[i].widget->getUrl();
            m_WidgetLayout.zones[i].zone.acct = m_WidgetLayout.zones[i].widget->getAcct();
            m_WidgetLayout.zones[i].zone.pass = m_WidgetLayout.zones[i].widget->getPass();
        }
        else
        {
            m_WidgetLayout.zones[i].zone.url = QString();
            m_WidgetLayout.zones[i].zone.acct = QString();
            m_WidgetLayout.zones[i].zone.pass = QString();
        }
...
void WidgetManager::initVideoWidget(VideoWidget * widget, WidgetZone & widgetZone)
{
    ...
    else
    {
        ...
        widgetZone.widget->setMediaInfo(widgetZone.zone.url,
